...
std::size_t expected_data_bytes(const VideoFecReassembler::PendingFrame& frame,
  ...
  const auto offset = index * kVideoFecShardPayloadBytes;
  if (offset >= frame.frame_bytes) {
    return 0;
  }
  return std::min<std::size_t>(kVideoFecShardPayloadBytes, frame.frame_bytes - offset);
}
...
std::optional<EncodedFrame> VideoFecReassembler::push_data(
  ...
  if (was_completed(shard.header.frame_id)) {
    return std::nullopt;
  }
  expire(now);
  ...
  if (!has_all_data(*pending)) {
    return std::nullopt;
  }
  auto result = complete(*pending, false);
  if (result) {
...
std::optional<EncodedFrame> VideoFecReassembler::push_parity(
  ...
  if (was_completed(decoded->header.frame_id)) {
    return std::nullopt;
  }
  expire(now);
  ...
  if (pending->parity_shards == 0 || present_count(*pending) < pending->data_shards) {
    return std::nullopt;
  }
  for (std::size_t index = 0; index < pending->data_shards; ++index) {
    auto& data = pending->shards[index];
    if (!data.present) {
      const auto size = expected_data_bytes(*pending, index);
      if (size == 0 || size > pending->shard_bytes) {
        return std::nullopt;
      }
  ...
  FecBlock block{{pending->data_shards, pending->parity_shards, pending->shard_bytes},
                 pending->shards};
  if (!codec_.recover(block)) {
    return std::nullopt;
  }
  pending->shards = std::move(block.shards);
  auto result = complete(*pending, true);
  if (result) {
    erase_frame(pending->frame_id);
  }
  return result;
...
std::vector<std::uint32_t> VideoFecReassembler::expire(SteadyClock::time_point now) {
  ...
  for (const auto id : insertion_order_) {
    ...
    if (found != frames_.end() && now >= found->second.deadline) {
      if (!has_all_data(found->second) && found->second.parity_shards != 0) {
...
void VideoFecReassembler::erase_frame(std::uint32_t frame_id) {
