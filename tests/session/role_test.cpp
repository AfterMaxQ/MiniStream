...
TEST_CASE("role state machine allows each session direction") {
  ...
  REQUIRE(valid_role_transition(RoleState::RemoteBrowsing, RoleState::RemoteConnecting));
  REQUIRE(valid_role_transition(RoleState::RemoteConnecting, RoleState::Pairing));
...
TEST_CASE("role state machine requires cleanup before changing direction") {
  ...
  REQUIRE_FALSE(valid_role_transition(RoleState::RemoteConnecting, RoleState::Streaming));
