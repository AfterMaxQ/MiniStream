...
TEST_CASE("handshake messages have deterministic validated wire formats") {
  const Hello hello{HandshakeRole::Controller, VideoCodec::Hevc, 3840, 2160, 60,
                    50'000'000, 0x0102030405060708ULL};
  auto encoded_hello = encode_hello(hello);
  REQUIRE(decode_hello(encoded_hello) == hello);

  const Accept accept{HandshakeRole::Controlled, 42, VideoCodec::Hevc, 3840, 2160, 60,
...
TEST_CASE("handshake retries HELLO every 250 ms and validates the nonce") {
  const Hello hello{HandshakeRole::Controller, VideoCodec::H264, 1920, 1080, 60,
                    20'000'000, 77};
  ...
  REQUIRE_FALSE(retrier.accept({HandshakeRole::Controlled, 1, VideoCodec::H264, 1920,
                               1080, 60, 20'000'000, 78}));
  REQUIRE(retrier.accept({HandshakeRole::Controlled, 1, VideoCodec::H264, 1920, 1080,
                          60, 20'000'000, 77}));
  REQUIRE_FALSE(retrier.next_hello(start + 1s));
}

TEST_CASE("handshake rejects messages sent by the wrong role") {
  const Hello hello{HandshakeRole::Controlled, VideoCodec::H264, 1920, 1080, 60,
                    20'000'000, 77};
  REQUIRE_FALSE(decode_hello(encode_hello(hello)));

  const Accept accept{HandshakeRole::Controller, 1, VideoCodec::H264, 1920, 1080, 60,
                      20'000'000, 77};
